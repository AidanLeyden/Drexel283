...
static uint8_t send_buff[RDSH_COMM_BUFF_SZ];
static uint8_t recv_buff[RDSH_COMM_BUFF_SZ];
...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        if (rc != OK) {
            char err_msg[] = "error: incorrect command or pipe formatting\n";
            send_message_string(cli_socket, err_msg);
            send_message_eof(cli_socket);
            continue;
        }
        if (cmd_list.num == 0) {
            char warn_msg[] = "warning: no commands provided\n";
            send_message_string(cli_socket, warn_msg);
            send_message_eof(cli_socket);
            continue;
        }
        Built_In_Cmds built_in = rsh_match_command(cmd_list.commands[0].argv[0]);
            ...
            switch (built_in) {
                ...
                case BI_CMD_CD:
                    if (cmd_list.commands[0].argc > 1) {
                        ...
                        send_message_string(cli_socket, "warning: no directory provided\n");
                    }
                    send_message_eof(cli_socket);
                    free_cmd_list(&cmd_list);
                    continue;
                case BI_CMD_RC: {
                    char response[256];
                    snprintf(response, sizeof(response), "rc: %d\n", last_rc);
        ...
        last_rc = WEXITSTATUS(pipeline_rc);
...
int send_message_string(int cli_socket, char *buff){
    int sent = send(cli_socket, buff, strlen(buff), 0);
    if (sent != (int)strlen(buff)) {
        return ERR_RDSH_COMMUNICATION;
    }
    // After sending the actual message, send the EOF indicator.
        ...
        if (pid < 0) {
            ...
            if (i == 0) {
                if (dup2(cli_sock, STDIN_FILENO) == -1) {
                    perror("dup2: cli_sock -> STDIN_FILENO");
                    _exit(1);
                }
            } else {
                if (dup2(pipes[i-1][0], STDIN_FILENO) == -1) {
                    perror("dup2: previous pipe -> STDIN_FILENO");
                    _exit(1);
                }
            }
            ...
            if (i == clist->num - 1) {
                if (!clist->commands[i].output_file) {
                    if (dup2(cli_sock, STDOUT_FILENO) == -1) {
                        perror("dup2: cli_sock -> STDOUT_FILENO");
                        _exit(1);
                    }
                    if (dup2(cli_sock, STDERR_FILENO) == -1) {
                        perror("dup2: cli_sock -> STDERR_FILENO");
                        _exit(1);
                    }
                }
            }
                ...
                if (dup2(pipes[i][1], STDOUT_FILENO) == -1) {
                    perror("dup2: current pipe -> STDOUT_FILENO");
                    _exit(1);
                }
            }
            ...
            dprintf(cli_sock, "error: %s: command not found\n", clist->commands[i].argv[0]);            fflush(stderr);
            _exit(1);
        }
        else {
            pids[i] = pid;
