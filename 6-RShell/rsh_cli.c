...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) 
    {
        ...
        if(sent_bytes != (ssize_t)strlen(cmd_buff)){
            perror("send");
        ...
        if (should_exit) {
            tv.tv_sec = 2;
            tv.tv_usec = 0;
            setsockopt(cli_socket, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv, sizeof tv);
        }
        
        while(!found_eof) {
            io_size = recv(cli_socket, rsp_buff + total_received, SH_CMD_MAX - total_received - 1, 0);
            
            if(io_size < 0) {
                if (should_exit && (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR)) {
                    break;
                }
                perror("recv");
                return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
            ...
            for(int i = 0; i < io_size; i++) {
                if(rsp_buff[total_received + i] == RDSH_EOF_CHAR) {
                    found_eof = 1;
                    
                    rsp_buff[total_received + i] = '\0';
                    printf("%s", rsp_buff);
                    fflush(stdout);
                    break;
                }
            ...
            if(!found_eof) {
                ...
                memset(rsp_buff, 0, total_received);
                total_received = 0;
            }
        }
